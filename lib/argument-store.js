class ArgumentStore {
  ...
  static defaults = {
    ...
    // if SEMAPHORE_AGENT_SUBNETS is set, but SEMAPHORE_AGENT_VPC_ID is not, SEMAPHORE_AGENT_SUBNETS will be ignored
  ...
  static fromEnv() {
    ...
    if (!argumentStore.isEmpty("SEMAPHORE_AGENT_VPC_ID")) {
      console.assert(
        !argumentStore.isEmpty("SEMAPHORE_AGENT_SUBNETS"),
        "SEMAPHORE_AGENT_SUBNETS is required, if SEMAPHORE_AGENT_VPC_ID is set."
      )
    }
