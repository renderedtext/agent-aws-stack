...
class ArgumentStore {
  ...
  static fromMap(params) {
    ...
    validateOverprovisionStrategy(argumentStore);
    return argumentStore;
  }

  validateOverprovisionStrategy(argumentStore) {
    const strategy = argumentStore.get("SEMAPHORE_AGENT_OVERPROVISION_STRATEGY")
    const factor = argumentStore.get("SEMAPHORE_AGENT_OVERPROVISION_FACTOR")
    switch (strategy) {
      case "none":
        return
      case "number":
        let n = parseInt(factor)
        if (isNaN(n)) {
        ...
        return
      case "percentage":
        let p = parseInt(factor)
        if (isNaN(p)) {
          throw "SEMAPHORE_AGENT_OVERPROVISION_FACTOR is invalid"
        }

        if (p < 1 || p > 100) {
          throw "SEMAPHORE_AGENT_OVERPROVISION_FACTOR must be between 1-100 when percentage strategy is used"
        }
