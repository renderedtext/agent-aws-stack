...
class AwsSemaphoreAgentStack extends Stack {
        ...
        configuration: [
          {
            ...
            parameters: [
              ...
              },
              {
                ...
                values: this.argumentStore.getAsList("SEMAPHORE_AGENT_MAC_FAMILY")
              },
              {
  ...
  createAutoScalingGroup(launchTemplate) {
    ...
    if (this.argumentStore.get("SEMAPHORE_AGENT_OS") == "macos") {
      const macFamilies = this.argumentStore.getAsList("SEMAPHORE_AGENT_MAC_FAMILY");
      mixedInstancesPolicy.launchTemplate.overrides = macFamilies.map(family => ({
        instanceType: `${family}.metal`
      }));
    } else if (!this.argumentStore.isEmpty("SEMAPHORE_AGENT_ASG_LAUNCH_TEMPLATE_OVERRIDES_JSON")) {
