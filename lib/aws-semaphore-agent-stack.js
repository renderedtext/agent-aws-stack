...
class AwsSemaphoreAgentStack extends Stack {
  ...
  createSecurityGroups() {
    ...
    // if key name is set, we allow inbound access
    if (!this.argumentStore.isEmpty("SEMAPHORE_AGENT_KEY_NAME")) {
      // Windows uses RDP which relies on port 3389
      if (this.argumentStore.get("SEMAPHORE_AGENT_OS") == "windows") {
        securityGroup.addIngressRule(Peer.anyIpv4(), Port.tcp(3389), 'allow rdp access from anywhere');
      } else {
  ...
  createLaunchConfiguration(ssmParameter, iamInstanceProfile, securityGroups) {
    ...
    if (!imageId) {
      const props = {
        name: `semaphore-agent-v${packageInfo.version}-${this.argumentStore.get("SEMAPHORE_AGENT_OS")}-amd64-server-${hash()}`,
        owners: [this.account]
      }
