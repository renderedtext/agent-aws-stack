...
class AwsSemaphoreAgentStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    this.createScalerLambda(defaultAWSSSMKey, autoScalingGroup);
  }
  ...
  createScalerLambda(defaultAWSSSMKey, autoScalingGroup) {
    const lambdaDependencies = new cdk.ConcreteDependable();
    lambdaDependencies.add(autoScalingGroup);

    let account = cdk.Stack.of(this).account;
    let lambdaRole = new iam.Role(this, 'scalerLambdaRole', {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
    ...
    const apiTokenParameterName = this.argumentStore.get("SEMAPHORE_API_TOKEN_PARAMETER_NAME");
    lambdaRole.addToPolicy(new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      actions: ["ssm:GetParameter"],
      resources: [
        `arn:aws:ssm:*:*:parameter/${apiTokenParameterName}`
      ]
    }))
    ...
    let lambdaFunction = new lambda.Function(this, 'ScaleAgentsLambdaFunction', {
      ...
      role: lambdaRole,
      environment: {
        "SEMAPHORE_API_TOKEN_PARAMETER_NAME": this.argumentStore.get("SEMAPHORE_API_TOKEN_PARAMETER_NAME"),
        "SEMAPHORE_AGENT_TYPE_NAME": this.argumentStore.get("SEMAPHORE_AGENT_TYPE_NAME")
      }
    });

    const lambdaRuleDependencies = new cdk.ConcreteDependable();
    lambdaRuleDependencies.add(lambdaFunction);

    const lambdaRule = new events.Rule(this, 'ScaleAgentsRule', {
    ...
    lambdaRule.node.addDependency(lambdaRuleDependencies);
