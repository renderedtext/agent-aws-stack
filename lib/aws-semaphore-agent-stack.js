...
class AwsSemaphoreAgentStack extends Stack {
  ...
  createAutoScalingGroup(launchTemplate) {
    ...
    if (!this.argumentStore.isEmpty("SEMAPHORE_AGENT_MIXED_POLICY_INSTANCE_REQUIREMENTS_JSON")) {
      const instanceRequirements = JSON.parse(this.argumentStore.get("SEMAPHORE_AGENT_MIXED_POLICY_INSTANCE_REQUIREMENTS_JSON"));
